// ==UserScript==
// @name		Freecast Site Search Print Backend Shortcut
// @include		*://watch.freecast.com/ai-search/search-results?prompt=*
// @include		*://watchstag.freecast.com/ai-search/search-results?prompt=*
// @include		*://watch.freecast.com/ai-search/search-results/?prompt=*
// @include		*://watchstag.freecast.com/ai-search/search-results/?prompt=*
// @version  	1
// @grant		addStyle
...
let startInterval = setInterval(function () {
	...
	let firstMediaBox = mediaBoxes.children[0];
	console.log(mediaBoxes);
	if (mediaBoxes.innerText.includes("TV Shows") || mediaBoxes.innerText.includes("Movies")) {
		let mediaSelection = ""
		if(firstMediaBox.innerText.includes("TV Shows") || mediaBoxes.innerText.includes("Movies")) {
			console.log("Logging: " + firstMediaBox.innerText)
			mediaSelection = mediaArrayMappings[firstMediaBox.innerText];
		}
		...
		for (childrenMedia of mediaBoxes.children) {
			childrenMedia.addEventListener("click", (thisObj) => {
				console.log(thisObj);
				console.log(thisObj.target.parentElement.innerText);
				currentMediaTargetClick = mediaArrayMappings[thisObj.target.parentElement.innerText];
			});
		}
	} else {
...
function getContentData() {
	...
	for (let i = 0; i < contentPanel.length; i++) {
		...
		adminDashboardLink = `https://stvstaging.freecast.com/admin/guide/${isVod}${
			currentMediaTargetClick == "movies" ? "movie" : "show"
		}/${targetResource[i].id}`;
